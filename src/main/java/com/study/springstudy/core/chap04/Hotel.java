...
public class Hotel {
     ...
     * @Autowired - 필드, 생성자. setter에 각각 등록 간으
     *              특정 객체가 다른 객체와의 의존성을 가질 때, 의존성 주입을 자동으로 처리해 주는 문법.
     *              생성자에 붙이는 것을 권장.
     * @qualifier - 특정 타입을 만족하는 빈이 2개 이상일 경우, 스프링이 자체적으로 무엇을 주입할 지 판단할 수 없기
     *              때문에, 별칭 등을 이용해서 주입할 빈을 직접 지목하는 문법.
...
//    @Qualifier("kim")
    ...
    @Autowired
    // 만약에 해당 클래스의 생성자가 단 한개 뿐이라면 자동으로
    ...
    // 호텔을 소개하는 기능
    public void inform() {
        System.out.printf("우리 호텔의 레스토랑은 %s입니다. 그리고 헤드 쉐프는 %s 입니다. \n",
